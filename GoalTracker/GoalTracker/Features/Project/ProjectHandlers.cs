...
namespace GoalTracker.Features.Project
{
    ...
    public class GetProjectsByScenHandler(IProjectRepository repository, AppMapper mapper)
       : IRequestHandler<GetProjectsByScenQuery, IEnumerable<ProjectDTO>>
    {
       
        public async Task<IEnumerable<ProjectDTO>> Handle(GetProjectsByScenQuery request, CancellationToken cancellationToken)
        {
            IEnumerable<GoalTracker.Domain.Entities.Project> projects;
            if (request.scenId.HasValue)
            {
                projects = await repository.GetByScenIdAsync(request.UserId, request.scenId.Value);
            }
            else
            {
                projects = await repository.GetAllAsync(request.UserId);
